...
public:
  ...
  int contLanzadas() { return lanzamientos; }
...
private:
  int id;
  int exploradorasEnUso;
  int exploradorasCoronadas;
public:
  Jugador() : id(0), exploradorasEnUso(0) {} 
  Jugador(int jugadorID) : id(jugadorID), exploradorasEnUso(0) {}
  void fichasCoronadas(int);
  void LimMaxExploradoras() {
    if (exploradorasEnUso < 3) {
      exploradorasEnUso++;
    } else {
      cout << "No se pueden tener más de 3 exploradoras en uso" << endl;
    }
  }
